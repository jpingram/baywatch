...
void Controller::sortTickets(){
    sort(ticketQueue.begin(), ticketQueue.end());
};

void Controller::updateBox(unsigned short index, std::string newL, long ticks){
    if(index < boxList.size()){
        //call set label function in selected box
        boxList[index].setLabel(newL);

        //set up time_point object using ticks parameter
        //set active start point of current box to new time point
        std::chrono::duration<long> activeStartPointDur(ticks);
        std::chrono::steady_clock::time_point newASP(std::chrono::steady_clock::now());
        newASP -= activeStartPointDur;
        boxList[index].setActiveStartPoint(newASP);
    }
};
